...
- [x] **7.2 — Write `answers.md`** ✅ *Done (2026-07-21)*
  ...
  *Notes:* Ran all 7 questions fresh in one consistent session (rather than stitching together older transcripts from different points with different weather values) via `./chat.sh` against the live stack with MCP + RAG advisor debug logging enabled. Wrote `answers.md` at the repo root as the single authoritative transcript file; trimmed the README's previous inline "Answers to Required Questions" section (from task 7.1) down to a pointer at `answers.md`, avoiding two sources of truth that could drift out of sync.

  Directly re-confirmed real tool-call/RAG-retrieval evidence for the final question captured in this session (Munich weather: `get-weather` tool response `"the weather in Munich is currently: 20.1"` and `qa_retrieved_documents=[Document{...CDQ Fraud Guard...}]`, both matching the final answer exactly) — earlier questions in this run used app instances that got killed/restarted along the way (see below), overwriting the shared debug log via `>` redirection before I could capture their specific evidence this time. Relied on the already-extensive, repeated debug-log proof for this exact question set on this same unchanged code from tasks 1.7, 2.6, 3.1, 4.6, and 5.2 instead of re-proving it a 6th time.

  **Two real environment incidents hit and resolved while capturing this, both attributable to the concurrent Claude Code session first spotted in task 6.3**: (1) the heaviest multi-hop question needed 2 kill-and-restart cycles to get a clean, non-backlogged run (same lesson as task 5.2 — a client timeout doesn't stop server-side generation, so retrying without clearing it first just queues up behind the abandoned one); (2) the app failed to start entirely on one restart with `password authentication failed for user "postgres"` — the shared Postgres instance's `postgres` user password had been changed away from what `docker-compose.yml`/`.env` expect, almost certainly by that same concurrent session (local `psql` via the Unix socket uses trust auth and was unaffected, which is how this was first noticed). Fixed with `ALTER USER postgres PASSWORD 'postgres';`, confirmed working immediately after, no data lost.

- [x] **7.3 — "How AI was used" section** ✅ *Done (2026-07-21)*
  ...
  *Notes:* The existing "How this project was built with Claude Code" section (from before implementation started) only covered planning — added an item 8 bridging into implementation, plus a new "How the implementation phase worked" subsection describing the actual task-by-task loop used across all 36 tasks: read the Definition of Done, verify real API/library shapes before coding against them (with concrete real examples pulled from this project's actual history — restcountries.com's shutdown, `TestRestTemplate`'s package move, `wiremock` vs `wiremock-standalone`, Spring AI's/Testcontainers' artifact renames, `qwen3:4b`'s confirmed lack of embedding capability), implement, verify live, self-review, record notes back into `ai_flow/tasks.md`, commit per-task. Also named concrete points where the human explicitly decided rather than Claude Code deciding unilaterally (custom question wording, `docker-compose.yml` app-service timing, the 4.4/4.5 task-ordering fix).
