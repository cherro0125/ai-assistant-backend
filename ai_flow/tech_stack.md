...
| Transport | stdio | This server has no documented HTTP/SSE mode — it's built for Claude Desktop's stdio-based MCP config, so the Spring AI MCP client connects to it as a stdio subprocess (`spring.ai.mcp.client.stdio.connections.weather`, command `weather-mcp/node_modules/.bin/tsx`, arg `weather-mcp/src/index.ts` — a direct relative path, not `npx tsx`: task 2.4 found `ServerParameters`/`StdioClientTransport` spawns with no way to set the subprocess's working directory, inheriting the JVM's own cwd, and `npx`'s local-package lookup only walks *upward* from cwd, so it would never find `weather-mcp/node_modules` sitting in a subdirectory — also **not** `npm start`, which prints npm's own banner to stdout ahead of the JSON-RPC stream and breaks the handshake, confirmed in task 2.2) |
...
| Integration tests (pgvector) | Testcontainers (`org.testcontainers:testcontainers-postgresql` — renamed from `postgresql` in the 2.x line, verified in task 4.7 — pointed at `pgvector/pgvector:pg17` image) | Docker confirmed available; avoids needing a real always-on DB for tests |
| Integration tests (MCP servers) | WireMock (`org.wiremock:wiremock-standalone`, not the bare `wiremock` artifact) for restcountries.com and weatherapi.com in tests | Keeps tests deterministic and offline; real MCP servers are exercised manually for the required Q&A, not in the automated test suite. **Note**: the bare `wiremock` artifact failed at runtime with "Jetty 11 is not present" — a newer Jetty version resolved elsewhere in the Boot 4.1 stack shadowed the one WireMock 3.x needs; `wiremock-standalone` shades its own Jetty and avoids the conflict |
